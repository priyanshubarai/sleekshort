...
exports.checkAlias = async (req,res,next) => {
    ...
    try{
        ...
        if (!entry) {
            ...
            return res.status(404).json({found: false, message: 'Short URL not found' });
        }else{
...
exports.getShortUrl = async (req,res,next) => {
    ...
    try{
        ...
        res.status(201).json({
            ...
            shortUrl: `http://localhost:7000/${newid}`
        });
...
exports.getShortUrlWithAlias = async (req,res,next) =>{
    ...
    try{
        ...
        console.log("new entry: ",newEntry)
        await newEntry.save()
        .then( res => {console.log("entry saved",res)})
        .catch(err=>{console.log(err)})
        console.log("saved in DB!")
        res.status(201).json({
            message: 'Short URL created',
